    ...
    <main className="min-h-screen bg-[#edf2ef] py-8">
      <div className="mx-auto w-[min(1280px,calc(100%-40px))] max-[620px]:w-[min(1280px,calc(100%-28px))]">
        ...
        <div className="grid grid-cols-[420px_1fr] gap-7 max-[1000px]:grid-cols-1">
          ...
          <section>
            <div className="mb-4 flex flex-wrap justify-end gap-3"><button onClick={() => setModalOpen(true)} className="inline-flex items-center gap-2 rounded-full border border-[#d5dfda] bg-white px-5 py-3 font-bold text-green"><FolderOpen size={18} /> Ver vouchers</button><button onClick={() => generatePdf(voucher)} className="inline-flex items-center gap-2 rounded-full bg-green px-5 py-3 font-bold text-white"><Download size={18} /> Gerar voucher em PDF</button></div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-[#071a15]/70 p-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="saved-vouchers-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setModalOpen(false); }}>
          <section className="max-h-[85vh] w-full max-w-2xl overflow-hidden rounded-[24px] bg-white shadow-2xl">
            <header className="flex items-center justify-between border-b border-[#e5ebe8] px-6 py-5"><div><span className="text-xs font-extrabold uppercase tracking-[.18em] text-green-light">Histórico</span><h2 id="saved-vouchers-title" className="mt-1 text-2xl font-bold">Vouchers salvos</h2></div><button onClick={() => setModalOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-[#f2f5f3] text-muted hover:text-ink" aria-label="Fechar"><X size={20} /></button></header>
            <div className="p-6">
              <input value={modalFilter} onChange={(event) => setModalFilter(event.target.value)} className="mb-5 w-full rounded-full border border-[#dce4e0] bg-[#f8faf9] px-5 py-3 text-sm outline-none focus:border-green-light" placeholder="Filtrar por nome ou código" aria-label="Filtrar vouchers por nome ou código" />
              <div className="max-h-[52vh] space-y-3 overflow-y-auto pr-1">
                {filteredSavedVouchers.map((item) => (
                  <article key={`${item.code}-${item.savedAt}`} className="flex items-center gap-4 rounded-2xl border border-[#e5ebe8] p-4">
                    ...
                    <button onClick={() => viewSavedVoucher(item)} className="grid h-10 w-10 place-items-center rounded-full border border-[#dce4e0] text-green hover:border-green-light" title="Visualizar voucher" aria-label={`Visualizar voucher ${item.code}`}><Eye size={18} /></button>
